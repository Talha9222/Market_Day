...
#if UNITY_EDITOR
    // When entering Play Mode with domain reload disabled, static state survives across sessions.
    // Drop the singleton so channel handlers registered in a previous session (closures capturing
    // now-destroyed UniWebView instances) do not accumulate.
    internal static void ResetForFastEnterPlayMode() {
        instance = null;
    }
#endif
