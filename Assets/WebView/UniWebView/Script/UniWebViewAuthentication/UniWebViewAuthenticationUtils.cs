...
public class UniWebViewAuthenticationUtils {
    internal static Dictionary<string, string> ParseFormUrlEncodedString(string input) {
        ...
        foreach (var pair in pairs) {
            ...
            // Split on the first '=' only: values are allowed to contain '=' (common in base64 data).
            var kv = pair.Split(new[] { '=' }, 2);
            var key = UnityWebRequest.UnEscapeURL(kv[0]);
            var value = kv.Length > 1 ? UnityWebRequest.UnEscapeURL(kv[1]) : "";
            // OAuth parameters are single-valued. Reject ambiguous input instead of deciding by
            // parameter order; the same decoded name must not appear more than once.
            if (result.ContainsKey(key)) {
                throw AuthenticationResponseException.InvalidResponse(input);
            }
    ...
    public static string GenerateCodeVerifier(int length = 64) {
        // The verifier must be generated directly from a cryptographically secure RNG. Seeding a
        // non-crypto `System.Random` with a few bytes would collapse the effective entropy to the
        // seed size and make the whole sequence reconstructible from the seed, which weakens PKCE
        // (RFC 7636 requires high-entropy verifiers). Map crypto bytes onto the unreserved character
        // set, using rejection sampling to avoid modulo bias.
        const string chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_.~";
        var result = new char[length];

        using var rng = RandomNumberGenerator.Create();
        // Largest multiple of the alphabet size that fits in a byte; values at or above it are
        // rejected so every character is drawn with equal probability.
