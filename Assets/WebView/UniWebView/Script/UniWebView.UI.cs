...
#pragma warning restore 618
    ...
    private Rect NativeFrameRect(Rect unityFrame) {
        ...
        return CalculateCocoaNativeFrameRect(
            unityFrame,
            Screen.width,
            Screen.height,
    ...
    // Cocoa receives physical-pixel coordinates and converts them to points using its backing
    // scale. Unity's render resolution may be lower than that physical viewport, so a
    // RectTransform frame needs this mapping before crossing the native boundary.
    private static Rect CalculateCocoaNativeFrameRect(
        Rect unityFrame,
