...
namespace UniWebViewExternal {
    ...
    public static class UniWebViewJsonUtility {
        ...
        private static T ParseSimpleType<T>(string json) {
            ...
            try {
                if (type == typeof(string)) {
                    ...
                    return (T)(object)int.Parse(json, CultureInfo.InvariantCulture);
                } else if (type == typeof(float)) {
                    return (T)(object)float.Parse(json, CultureInfo.InvariantCulture);
                } else if (type == typeof(double)) {
                    return (T)(object)double.Parse(json, CultureInfo.InvariantCulture);
                } else if (type == typeof(bool)) {
                    return (T)(object)bool.Parse(json);
                } else if (type == typeof(long)) {
                    return (T)(object)long.Parse(json, CultureInfo.InvariantCulture);
                } else if (type == typeof(short)) {
                    return (T)(object)short.Parse(json, CultureInfo.InvariantCulture);
                } else if (type == typeof(byte)) {
                    return (T)(object)byte.Parse(json, CultureInfo.InvariantCulture);
        ...
        // Single-pass unescape for the small set of escapes produced by ConvertSimpleTypeToJson
        // (\" \\ \n \r \t). A chained String.Replace is wrong here: unescaping "\\" to "\" first can
        // turn a literal "\\n" (backslash + n) into a newline on a later pass.
        ...
        private static string ConvertSimpleTypeToJson(object obj) {
            ...
            if (type == typeof(string)) {
                ...
                // Format with the invariant culture so the JSON always uses '.' as the decimal
                // separator, regardless of the app's current culture. The old Replace(",", ".")
                // hack produced invalid JSON under cultures that use '.' as a thousands separator.
