...
public class UniWebViewInterface {
    ...
    // Keep strong managed references to the callbacks handed to native. Native holds these function
    // pointers for the whole app lifetime; without a rooted reference the marshalled thunk can be
    // garbage collected (notably under Mono in the macOS Editor), leaving native with a dangling
    // pointer that crashes on the next callback.
    private static readonly UnitySendMessageDelegate sendMessageDelegate = SendMessage;
    private static readonly ChannelMethodDelegate channelMethodDelegate = ChannelFunc;
    private static readonly ChannelMethodResultReleaseDelegate channelMethodResultReleaseDelegate = ReleaseChannelResult;
    ...
    static void ConnectMessageSender() {
        ...
        CheckPlatform();
        uv_connectMessageSender(sendMessageDelegate);
    }
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    delegate IntPtr ChannelMethodDelegate(IntPtr namePtr, IntPtr methodPtr, IntPtr parameterPtr);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    delegate void ChannelMethodResultReleaseDelegate(IntPtr resultPtr);

    [DllImport(DllLib)]
    private static extern void uv_registerChannel(
        [MarshalAs(UnmanagedType.FunctionPtr)] ChannelMethodDelegate channel,
        [MarshalAs(UnmanagedType.FunctionPtr)] ChannelMethodResultReleaseDelegate release
    );
    public static void RegisterChannel() {
        UniWebViewLogger.Instance.Info("Connecting to native side method channel.");
        CheckPlatform();
        uv_registerChannel(channelMethodDelegate, channelMethodResultReleaseDelegate);
    }

    [MonoPInvokeCallback(typeof(ChannelMethodDelegate))]
    private static IntPtr ChannelFunc(IntPtr namePtr, IntPtr methodPtr, IntPtr parameterPtr) {
        // This runs as a native->managed callback. An exception must never escape here: crossing the
        // reverse-P/Invoke boundary aborts the process under IL2CPP. Catch everything and return null.
        try {
            string name = Marshal.PtrToStringAuto(namePtr);
            string method = Marshal.PtrToStringAuto(methodPtr);
            string parameters = Marshal.PtrToStringAuto(parameterPtr);
            ...
            if (name == GlobalChannelIdentifier) {
                ...
                result = UniWebViewStaticListener.InvokeStaticMethod(method, parameters);
            } else {
                UniWebViewLogger.Instance.Verbose(() => $"ChannelFunc invoked by native side. Name: {name} Method: {method} Params: {parameters}");
                result = UniWebViewChannelMethodManager.Instance.InvokeMethod(name, method, parameters);
            }

            return AllocateChannelResult(result);
        } catch (System.Exception e) {
            UniWebViewLogger.Instance.Critical(
                "Critical error in ChannelFunc: " + e.Message + "\nStackTrace: " + e.StackTrace
            );
            return IntPtr.Zero;
        }
    }

    private static IntPtr AllocateChannelResult(string value) {
        if (value == null) {
            return IntPtr.Zero;
        }

        byte[] bytes = Encoding.UTF8.GetBytes(value);
        IntPtr resultPtr = Marshal.AllocHGlobal(bytes.Length + 1);
        try {
            Marshal.Copy(bytes, 0, resultPtr, bytes.Length);
            Marshal.WriteByte(resultPtr, bytes.Length, 0);
            return resultPtr;
        } catch {
    ...
    [MonoPInvokeCallback(typeof(ChannelMethodResultReleaseDelegate))]
    private static void ReleaseChannelResult(IntPtr resultPtr) {
